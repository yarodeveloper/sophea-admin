...
try {
    ...
    $id = 3; // Contugen
    echo "--- ANALYSIS FOR CLIENT ID $id (Contugen) --- \n";
    ...
    $stmt = $pdo->prepare("SELECT s.*, 
                           (SELECT SUM(amount) FROM payments WHERE service_id = s.id AND status = 'paid') as total_paid
                           FROM services s WHERE client_id = ?");
    $stmt->execute([$id]);
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo "Service: ID={$row['id']}, Name={$row['service_name']}, Status={$row['status']}, Progress={$row['progress_percentage']}%, Total Paid={$row['total_paid']}\n";
    }
